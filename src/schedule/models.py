...
def make_occurrence_id(
    summary: str, start_dt: datetime, end_dt: datetime, source_id: str, class_id: str
) -> str:
    identity = "\0".join((summary, start_dt.isoformat(), end_dt.isoformat(), source_id, class_id))
    digest = hashlib.sha256(identity.encode("utf-8")).hexdigest()[:16]
    return f"{slugify(summary)}--{start_dt.date().isoformat()}--{digest}"
...
class ClassMatchState:
    ...
    alternate_date_iso: str | None
    alternate_date_formatted: str | None
